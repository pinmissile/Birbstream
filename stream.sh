...
do
  hour=$(date +\"%H\")
  if [[ "$hour" =~ ^(21|22|23|00|01|02|03|04|05|06)$ ]]; then
    if [ $day == "1" ]
    then
      killall ffmpeg
      steaming="0"
    ...
    day="0"
    stream_cmd="-f video4linux2 -s 1820x1080 -framerate 24 -i /dev/video1 -c:v h264_omx -threads 0 -an -f flv"
  else
    if [ $day == "0" ]
      then
      killall ffmpeg
  ...
  if [[ $? -eq 0 ]] ; then
    ...
    then
      ...
      sleep 60
      ../FFMpeg/ffmpeg $stream_cmd $stream_url\"$live_key\" &
      streaming="1"
    fi
  elif [ $streaming -eq "1" ]
  then
